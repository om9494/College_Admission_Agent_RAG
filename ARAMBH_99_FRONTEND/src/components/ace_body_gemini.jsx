...
export default function AceBody() {
  ...
  const getdata = async (instructionsUrl, dbdataUrl) => {
    try {
      ...
      return instructions + dbdata;  // Or any other way you want to merge
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const mergedContent = await getdata(instructionsUrl, dbdataUrl);
  ...
  //   const targetApiLang = e.target.value;
  //   setLanguage(targetApiLang);
  ...
  //     const translatedApiText = await translateApiText(key, targetApiLang);
  //     // console.log("Translated text:", translatedText);
  //     el.innerText = translatedApiText;
  //   });
  // };
  ...
  const [transcript, setTranscript] = useState(""); // Initialize as empty


  // useEffect(() => { // Update placeholder and greeting when language changes
  //   const translatedHello = translate("Hello");
  //   setTranscript(translatedHello + " ✨");
  // }, [language, translate]); // Dependencies: language and translate
  ...
  const responseTextFormatter = async (response) => {
    ...
    // //Extract the text within the bot-text div using regex
    // let match = /<div[^>]*id="bot-text"[^>]*>(.*?)<\/div>/s.exec(response);  // s flag for . to match newlines
    // let textToTranslate = match ? match[1] : response; // Use match group or entire response if no match
    // textToTranslate = textToTranslate.replace(/(<\/div>)/g, "");
    // console.log("Text To Translate: " + textToTranslate);

    // let translatedText;
    // try {
    //   translatedText = await translateText(textToTranslate, language);
    // } catch (error) {
    //   console.error("Translation error in responseTextFormatter:", error);
    //   translatedText = textToTranslate; // Fallback to original text if translation fails
    // }
  ...
  const handleSend = async () => {
    try {
      ...
      const formattedResponse = "<div className='bot-text' id='bot-text' data-translate-key>" + await responseTextFormatter(botResponse) + "</div>";
      ...
      <div id='ace-head'>
        ...
        <div>
          {parsedUser ? (
            ...
            <>
              <button className="" onClick={() => navigate("/login")} > {translate("Login")} </button>
            </>
          )}
        ...
        {parsedAdmin ? (
          ...
          <>
            <label name="Notification" title="Notification and Email Services" className="switch">
              <p id="status"></p>
              <input type="checkbox" checked={notification} onChange={handleNotification} disabled/>
        ...
        <label name="Theme" title="Theme Switch" className="switch">
          <p id="status"></p>
          <input type="checkbox" checked={isDarkMode} onChange={handleDarkModeToggle} disabled />
